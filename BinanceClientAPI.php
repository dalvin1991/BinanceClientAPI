...
class API {
	...
	//Unsigned & Signed Request method
	private function unsignedRequest($url, $params = [], $method = "GET") {
		$opt = [
			"http" => [
				"method" => $method,
		...
		if (!$response = file_get_contents($this->base.$url.'?'.$query, false, $context)) 
		{
		  return "Error Occur. Please make sure parameter you pass is correct.";
		} else {
	...
	private function signedRequest($url, $params = [], $method = "GET") {
		...
		if (!$response = file_get_contents($endpoint, false, $context)) 
		{
		  return "Error Occur. Please make sure parameter you pass is correct.";
		} else {
			return json_decode($response,true);
	...
	public function buyOrder($symbol, $quantity, $price, $type = "LIMIT", $flags = []) {
		if($quantity<=0) return "Invalid Order Quantity Submitted.";
		return $this->submitOrder("BUY", $symbol, $quantity, $price, $type, $flags);
	}
	
	public function sellOrder($symbol, $quantity, $price, $type = "LIMIT", $flags = []) {
		if($quantity<=0) return "Invalid Order Quantity Submitted.";
		return $this->submitOrder("SELL", $symbol, $quantity, $price, $type, $flags);
	}
	...
	public function getOrders($symbol, $limit = 500) {
		return $this->signedRequest("v3/allOrders",["symbol"=>$symbol, "limit"=>$limit]);
	}
	
	public function getHistoryOrders($symbol) {
		return $this->signedRequest("v3/myTrades",["symbol"=>$symbol]);
	}
	
	public function getPrices() {
		return $this->getPriceData($this->unsignedRequest("v1/ticker/allPrices"));
	}
	
	public function getBookPrices() {
		return $this->getBookPriceData($this->unsignedRequest("v1/ticker/allBookTickers"));
	}
	...
	public function getPrevDayChange($symbol) {
		return $this->unsignedRequest("v1/24hr",["symbol"=>$symbol]);
	}
	
	public function getAggTrades($symbol) {
		return $this->getTradesData($this->unsignedRequest("v1/aggTrades",["symbol"=>$symbol]));
	}
	
	public function getDepth($symbol) {
		$json = $this->unsignedRequest("v1/depth",["symbol"=>$symbol]);
	...
	public function getBalances($priceData = false) {
		return $this->getBalanceData($this->signedRequest("v3/account"),$priceData);
	}
	
	private function getApiRequest($url, $method = "GET") {
		$opt = [
			"http" => [
				"method" => $method,
				"header" => "User-Agent: Mozilla/4.0 (compatible; PHP Binance API)\r\nX-MBX-APIKEY: {$this->api_key}\r\n"
	...
	public function submitOrder($side, $symbol, $quantity, $price, $type = "LIMIT", $flags = []) {				
		$opt = [
			"symbol" => $symbol,
			"side" => $side,
			"type" => $type,
	...
	public function getCandleSticks($symbol, $interval) {
		if ( !isset($this->charts[$symbol]) ) $this->charts[$symbol] = [];
		$response = $this->unsignedRequest("v1/klines", ["symbol"=>$symbol, "interval"=>$interval]);
		$ticks = $this->getChartData($symbol, $interval, $response);
	...
	private function getBalanceData($array, $priceData = false) {
		if ( $priceData ) $btc_value = 0.00;
		$balances = [];
		foreach ( $array['balances'] as $obj ) {
			$asset = $obj['asset'];
			$balances[$asset] = ["available"=>$obj['free'], "reserved"=>$obj['locked'], "btcValue"=>0.00000000];
			if ( $priceData ) {
	...
	private function getChartData($symbol, $interval, $ticks) {
		if ( !isset($this->info[$symbol]) ) $this->info[$symbol] = [];
	...
	private function getTradesData($trades) {
		$output = [];
	...
	private function getBookPriceData($array) {
		$bookprices = [];
		foreach ( $array as $obj ) {
			$bookprices[$obj['symbol']] = [
				"bid_price"=>$obj['bidPrice'],
				"bid_qty"=>$obj['bidQty'],
				"ask_price"=>$obj['askPrice'],
				"asks_qty"=>$obj['askQty']
			];
		}
		return $bookprices;
	...
	public function getTrades($symbols, $callback) {
		foreach ( $symbols as $symbol ) {
			if ( !isset($this->info[$symbol]) ) $this->info[$symbol] = [];
	...
	public function getChart($symbols, $interval, $callback) {
		foreach ( $symbols as $symbol ) {
			if ( !isset($this->charts[$symbol]) ) $this->charts[$symbol] = [];
			...
			$this->getCandleSticks($symbol, $interval);
	...
	public function userData($balance_callback, $execution_callback = false) {
		$response = $this->getApiRequest("v1/userDataStream", "POST");
		...
		$loop->addPeriodicTimer(30, function() use ($listenKey) {
			$this->getApiRequest("v1/userDataStream?listenKey={$listenKey}", "PUT");
		});
		$loop->run();
	}
